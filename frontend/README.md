...
- [ ] `files.html`에서 jsTree 트리가 기본 펼침으로 로드되고 `target/`/`.git` 등이 안 보이며, 폴더가 파일보다 먼저 정렬되고 폴더/파일 아이콘이 표시되는지(개발자 도구 네트워크 탭에서 `assets/vendor/` 아래 jQuery/jsTree/이미지가 모두 200으로 로드되는지도 확인)
