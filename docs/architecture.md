...
임의의 Maven(Java) 프로젝트를 Git URL 또는 ZIP으로 입력받아, **Java 21 / Spring Boot 4.1 / Spring Cloud 2025.1 / Spring AI 2**로 단계적으로 마이그레이션하고(1단계, 필수), 이후 남은 개별 OSS 취약점을 스캔·패치하는(2단계, 옵션) 로컬 실행 도구. 결과는 diff/리포트/AI 인수인계 가이드로 산출되며, **자동 커밋/푸시는 하지 않는다.**
