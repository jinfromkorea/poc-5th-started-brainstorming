...
| `status` | VARCHAR | NOT NULL | `"queued"` | `JOB_STATUSES = ("queued", "running", "awaiting_version_approval", "awaiting_approval", "success", "stage1_needs_handoff", "stage2_needs_handoff", "failed", "cancelled")` 중 하나. DB 레벨 CHECK 제약이나 enum 타입은 쓰지 않는다 — SQLite에 네이티브 enum이 없고, 마이그레이션 없이 상태값을 늘릴 수 있게 하려는 의도적 선택(코드 주석 참고). `awaiting_version_approval`(Stage 0 완료, 출력 버전 확인 대기)과 `awaiting_approval`(1단계 인수인계 + 2단계 승인 대기)은 의도적으로 비종료 상태다. `stage1_needs_handoff`/`stage2_needs_handoff`는 원래 하나였던 `needs_handoff`를 어느 단계가 막았는지 구분할 수 있게 분리한 값이다([`architecture.md`](architecture.md) §7.4, 스펙: `docs/superpowers/specs/2026-08-11-job-status-stage-split-design.md`). `TERMINAL_JOB_STATUSES = {"success", "stage1_needs_handoff", "stage2_needs_handoff", "failed", "cancelled"}`가 종료 상태 집합. |
