...
- **Stage 1만 대상으로 한다.** `job.run_stage1=true AND job.run_stage2=false`인 job에서만 이 기능을 노출/허용한다 — `needs_handoff`가 Stage 1 자체의 실패인지, Stage 2의 개별 CVE 실패인지 DB에 구분 저장돼 있지 않고, Stage 1+2를 같이 선택한 job은 두 경우가 섞일 수 있어서 이번 범위에서는 확실히 구분되는 경우만 다룬다.
...
- **게이트**: `job.status == "needs_handoff" AND job.run_stage1 == true AND job.run_stage2 == false`일 때만 허용. 그 외는 409.
- **새 job 상태를 만들지 않는다.** `needs_handoff`는 `TERMINAL_JOB_STATUSES`에 그대로 둔다 — "재개 가능한 터미널 상태"라는 개념 자체는 기존에도 없던 게 아니라(사람이 이력에서 삭제할 수도 있어야 함), 이번엔 그 위에 재개 액션 하나를 얹는 것뿐이다.
...
- **검증 실패 시**: 아무것도 커밋하지 않고 바로 `needs_handoff`로 되돌아간다. `output/handoff/stage1-guide.md`를 최신 빌드 출력으로 덮어써서, 사람이 뭐가 아직 안 되는지 바로 볼 수 있게 한다. 재개 버튼은 계속 남아있어 반복 시도 가능하다.
- **리포트는 이어붙인다**: 기존 `job.report_markdown`(1차 시도까지의 내용) 뒤에 이번 재개의 결과를 `\n\n---\n\n`로 이어붙인다(`run_pipeline_resume_stage2`가 이미 쓰는 패턴과 동일).
- **재개가 성공적으로 다 끝나도, 만약 그사이 새로운 스텝에서 또 막히면 다시 `needs_handoff`(새 가이드로)** — 반복 가능한 사이클이다.
...
async def run_pipeline_resume_stage1_after_handoff(
    ...
    status="needs_handoff" with run_stage1=true, run_stage2=false. work_dir
    is whatever a human left it as after manually fixing the code that
    blocked Stage 1 -- see docs/superpowers/specs/2026-08-11-stage1-
    handoff-resume-design.md."""
    ...
    try:
        ...
        if not ok:
            ...
            await set_job_status(session_factory, job_id, "needs_handoff")
            await emit("status", {"status": "needs_handoff"})
            return
        ...
        final_status = "needs_handoff" if stage1_result.status == "needs_handoff" else "success"
...
async def resume_stage1(
    ...
    Only for run_stage1-only jobs -- see spec for why Stage 2 combos are
    excluded."""
    job = db.get(Job, job_id)
    if job is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"unknown job_id: {job_id}")
    if job.status != "needs_handoff" or not job.run_stage1 or job.run_stage2:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"job {job_id} is not resumable (status={job.status}, run_stage1={job.run_stage1}, run_stage2={job.run_stage2})",
        )
...
- `status` 이벤트(SSE)는 `run_stage1`/`run_stage2`를 안 실어온다(다른 상태 이벤트 payload와 마찬가지). 대신 `job-view.js`에 모듈 수준 변수 `currentJobRunStage1`/`currentJobRunStage2`를 두고, 페이지가 job 데이터를 처음 알게 되는 시점에 채워두는 함수 `setJobStageFlags(runStage1, runStage2)`를 노출한다:
  - `index.html`(`assets/app.js`의 제출 핸들러): `POST /jobs` 응답으로 `job_id`를 받는 시점에 폼 체크박스 값(`runStage1Checkbox.checked`, `runStage2Checkbox.checked`)으로 바로 호출.
  - `job.html`(`assets/job.js`의 `loadJob()`): `GET /jobs/{id}` 응답 바디(`JobStatusResponse`, 이미 `run_stage1`/`run_stage2` 포함)로 호출.
- `status` 이벤트 핸들러: `data.status === "needs_handoff" && currentJobRunStage1 && !currentJobRunStage2`이면 "인수인계 후 재개" 버튼을 보여주고, 그 외 상태로 바뀌면 숨긴다.
...
- `needs_handoff`가 아니거나 `run_stage2=true`인 job에 `POST /jobs/{id}/resume-stage1` → 409.
- 존재하지 않는 job_id → 404.
...
- 재개가 성공해서 나머지 계획까지 전부 끝났는데 그 중간에 또 다른 스텝이 막히면 → 다시 `needs_handoff`, 재개 버튼 그대로 다시 노출.
...
- `test_resume_stage1_rejected_when_not_needs_handoff` — 아무 상태에서나 시도 시 409.
- `test_resume_stage1_rejected_when_run_stage2_true` — Stage 2도 선택된 job은 `needs_handoff`여도 409.
- `test_resume_stage1_verify_fails_stays_needs_handoff` — `mvn_test_compile` 실패 mock → 상태 그대로 `needs_handoff`, `mvn_effective_pom`/`run_stage1_migration`은 호출 안 됨(재분석/재계획 자체를 안 탐).
...
- `test_resume_stage1_verify_succeeds_but_next_step_blocks` — 재개 후 다음 스텝에서 또 막히는 경우 → 다시 `needs_handoff`, 새 handoff 가이드 파일 내용이 갱신되는지.
