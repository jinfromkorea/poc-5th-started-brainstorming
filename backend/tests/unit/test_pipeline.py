...
async def test_stage1_only_success_writes_report_and_diff(monkeypatch, settings, db, job_paths):
    ...
    async def fake_stage1(job_id, work_dir, detected, baseline, settings_):
        return MigrationRunResult(
            plan=MigrationPlan(steps=[], skipped=[]),
            outcomes=[],
            status="no_gap",
            final_diff="",
...
async def test_stage2_only_with_vulnerabilities(monkeypatch, settings, db, job_paths):
    ...
    async def fake_stage2(job_id, work_dir, vulns, baseline, settings_):
